...
static void process_arithmetic_cmd(LIBEVENT_THREAD *t, mcp_parser_t *pr, mc_resp *resp, const enum op_type op) {
    ...
    switch(arithmetic_cmd(t, key, nkey, op, delta, temp, NULL)) {
    case OK:
...
static void process_marithmetic_cmd(LIBEVENT_THREAD *t, mcp_parser_t *pr, mc_resp *resp) {
    ...
    bool item_created = false;
    uint64_t cas = 0;
    switch(do_arithmetic(t, key, nkey, op, of.delta, tmpbuf, &of.req_cas_id, hv, &it)) {
    case OK:
        //if (c->noreply)
        //    resp->skip = true;
...
int mcplib_internal_run(lua_State *L, conn *c, mc_resp *top_resp, int coro_ref) {
    ...
    switch (rq->pr.command) {
        ...
        case CMD_REPLACE:
            ...
            break;
        case CMD_INCR:
            process_arithmetic_cmd(t, pr, resp, ADD);
            break;
        case CMD_DECR:
            process_arithmetic_cmd(t, pr, resp, DEC);
            break;
        case CMD_MULT:
