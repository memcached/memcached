...
typedef struct io_queue_cb_s {
    void *ctx;                   // 未修改的指针，用于特定上下文
    io_queue_stack_cb submit_cb; // 一次提交完整的IO堆栈时调用的回调函数
    int type;                    // 类型标识
} io_queue_cb_t;
...
struct thread_notify {
    struct event notify_event;  /* 监听通知管道或eventfd的事件 */
#ifdef HAVE_EVENTFD
    int notify_event_fd;        /* 通知计数器 */
#else
    int notify_receive_fd;      /* 通知管道的接收端 */
    int notify_send_fd;         /* 通知管道的发送端 */
#endif
};
...
typedef struct {
    ...
    struct event_base *base;           /* 该线程使用的libevent句柄 */
    struct thread_notify n;            /* 用于线程通知的对象 */
    struct thread_notify ion;          /* 用于线程IO对象通知的对象 */
    pthread_mutex_t ion_lock;          /* 用于ion_head的互斥锁 */
    iop_head_t ion_head;               /* 用于IO对象返回的队列 */
    int cur_sfd;                       /* 用于记录命令的客户端fd */
    int thread_baseid;                 /* 数据偏移的线程编号 */
    struct thread_stats stats;         /* 该线程生成的统计信息 */
    io_queue_cb_t io_queues[IO_QUEUE_COUNT]; /* IO队列回调函数数组 */
    struct conn_queue *ev_queue;       /* 工作线程/连接事件队列 */
    cache_t *rbuf_cache;               /* 静态大小的读缓冲区 */
    mc_resp_bundle *open_bundle;       /* 打开的bundle */
    cache_t *io_cache;                 /* IO对象缓存 */
#ifdef EXTSTORE
    void *storage;                     /* 存储系统的数据对象 */
#endif
    logger *l;                         /* 日志缓冲区 */
    void *lru_bump_buf;                /* 异步LRU增加缓冲区 */
#ifdef TLS
    char   *ssl_wbuf;                  /* SSL写缓冲区 */
#endif
    int napi_id;                       /* 与该线程关联的napi id */
#ifdef PROXY
    void *proxy_ctx;                   /* 代理全局上下文 */
    void *L;                           /* Lua虚拟机 */
    void *proxy_hooks;                 /* 代理钩子 */
    void *proxy_user_stats;            /* 代理用户统计信息 */
    void *proxy_int_stats;             /* 代理内部统计信息 */
    void *proxy_event_thread;          /* 代理IO线程 */
    struct event *proxy_gc_timer;      /* 定期GC推送计时器 */
    pthread_mutex_t proxy_limit_lock;  /* 代理限制锁 */
    uint64_t proxy_active_req_limit;   /* 代理活动请求限制 */
    uint64_t proxy_buffer_memory_limit; /* 缓冲区内存限制（受limit_lock保护） */
    uint64_t proxy_buffer_memory_used; /* 缓冲区内存使用（受limit_lock保护） */
    uint32_t proxy_rng[4];             /* Lua的每线程快速随机数发生器 */
    // TODO: 添加ctx对象，以便我们可以附加到队列。
#endif
} LIBEVENT_THREAD;
...
struct _io_pending_t {
    int io_queue_type;                 // IO队列的类型，匹配IO_QUEUE_*之一
    LIBEVENT_THREAD *thread;           // 关联的工作线程
    conn *c;                           // 连接对象
    mc_resp *resp;                     // 关联的响应对象
    io_queue_cb return_cb;             // 完成时在工作线程上调用的回调
    io_queue_cb finalize_cb;           // 在工作线程上在最终化期间调用的回调
    STAILQ_ENTRY(_io_pending_t) iop_next; // IO挂起队列链中的下一个项目
    char data[120];                    // 用于IO操作的附加数据（缓冲区等）
};
