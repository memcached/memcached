...
static void do_slabs_stats(ADD_STAT add_stats, void *c) {
    ...
    for(i = POWER_SMALLEST; i <= power_largest; i++) {
        ...
        if (p->slabs != 0) {
            ...
            APPEND_NUM_STAT(i, "lease_hits", "%llu",
                                (unsigned long long)thread_stats.slab_stats[i].lease_hits);
            APPEND_NUM_STAT(i, "lease_badval", "%llu",
                                (unsigned long long)thread_stats.slab_stats[i].lease_badval);
            APPEND_NUM_STAT(i, "lease_set", "%llu",
                                (unsigned long long)thread_stats.slab_stats[i].lease_set)
